...
def main(path_to_fashion_MNIST_dataset = '/tmp/fashionmnist', 
    ...
    train_dataset = FashionMNISTProcessedDataset(path_to_fashion_MNIST_dataset, train=True)
    val_dataset = FashionMNISTProcessedDataset(path_to_fashion_MNIST_dataset, train=False)
    ...
    for epoch in range(num_epochs):
        ...
        for x, y_truth in train_loader:
            ...
            for val_x, val_y_truth in validation_loader:
                ...
                validation_loss_list.append(objective(val_y_hat, val_y_truth)) #Calculate the loss for the validation

            validation_losses.append( (sum(validation_loss_list)/float(len(validation_loss_list))).item() )

            cntr += 1

            loop.set_description( f'epoch:{epoch} batch:{batch} loss:{loss.item()} val_loss:{validation_losses[-1]}' )
    ...
    if visualize_training==True:
        plot_fig = plot_training_and_validation(train_losses, validation_losses)
